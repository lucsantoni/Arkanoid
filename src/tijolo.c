    ...
    while (fgets(linha, sizeof(linha), f) && linha_index < linhas)
    {
        for (int coluna_index = 0; coluna_index < colunas; coluna_index++)
        {
            ...
            if (tipo > 0)
            {
                ...
                t[i].ativo = 1;
                t[i].powerup = 0;
                if (tipo >= 1 && tipo <= 4) { // Apenas tijolos do tipo 1 a 4 podem ter power-ups
                    if ((rand() % 100) < 30) { // 30% de chance de ter um power-up
                        t[i].powerup = (rand() % 3) + 1; // 1: Fireball, 2: Extra Balls, 3: Platform Expand
                    }
                }
                i++;
            }
